...
class Julia extends Boss {
  ...
  void drawCharacter() {
    ...
    if (deathSequenceBoss == -1) {
      playAnimation(juliaAnimation, 12, charWidth, charHeight, new PVector(-charWidth/2, -charHeight/2));
    }
    if (deathSequenceBoss > 0) {
      deathSequenceBoss--;
      playAnimation(juliaLossAnimation, 12, charWidth, charHeight, new PVector(-charWidth/2, -charHeight/2));
    }
  ...
  void update() {
    ...
    if (frameCount % 60 == 0 && health > 0) {
      ...
      stroke(255, 0, 0);
      for (int i = 0; i < 3; i++) {
        bossProjectiles.add(new Projectiles(new PVector(pos.x, pos.y+20), new PVector(-5, spray), novel, new PVector(50, 70)));
        spray+=2;
        if (spray == 4) {
          spray = -4;
        }
